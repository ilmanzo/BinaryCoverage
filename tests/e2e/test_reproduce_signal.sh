#!/bin/bash
# E2E test to reproduce the signal forwarding / socket release issue (#143)
set -euo pipefail
...
# Create workspace
WORKDIR=$(mktemp -d /tmp/funkoverage_reproduce_signal_XXXX)
cd "$WORKDIR"
...
if kill -0 "$CHILD_PID" 2>/dev/null; then
    echo -e "  ${RED}REPRODUCED${NC}: Child daemon PID $CHILD_PID is STILL RUNNING!"
    info "Killing orphaned child daemon..."
    kill -9 "$CHILD_PID" 2>/dev/null || true
    echo ""
    echo "This reproduces issue #143: the shim parent exits without forwarding SIGTERM to the child."
    exit 0
else
